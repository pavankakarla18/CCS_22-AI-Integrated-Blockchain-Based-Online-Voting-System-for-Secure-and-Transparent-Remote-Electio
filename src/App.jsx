...
import {
    collection, onSnapshot, addDoc, deleteDoc, doc, setDoc, getDoc,
    Timestamp, updateDoc, query, orderBy, serverTimestamp
...
import { formatFirestoreTimestamp } from './time';

// --- ADMIN / UI IMPORTS ---
import {
    LockIcon, UserIcon, PowerIcon, LoaderIcon, ShieldCheckIcon,
    PlusCircleIcon, UsersIcon, TrashIcon, ArrowLeftIcon, AnnounceIcon,
    ShieldAlertIcon // 🆕 new icon replacing AlertTriangleIcon
...
const providerOptions = {
    walletconnect: {
        ...
        options: { infuraId: WALLETCONNECT_PROJECT_ID }
    }
};
let web3Modal;
...
// --- Helper: Voter ID <-> Fake Email mapping ---
const VOTER_EMAIL_DOMAIN = 'voter.local';
function voterIdToEmail(voterIdValue) {
    const cleanId = (voterIdValue || '').trim();
    return `${cleanId}@${VOTER_EMAIL_DOMAIN}`;
}
function extractVoterIdFromUser(userObj) {
    if (!userObj) return '';
    if (userObj.voterId) return userObj.voterId;
    if (userObj.email && userObj.email.endsWith(`@${VOTER_EMAIL_DOMAIN}`)) {
...
function LoginPage({ onLogin, onSignUp, loading }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (isLogin) onLogin(voterId, password);
        else onSignUp(voterId, password);
    };
        ...
        <div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <input
                    ...
                    className="w-full p-3 bg-gray-700 rounded-md text-white"
                />
                <button type="submit" disabled={loading} className="w-full py-3 bg-indigo-600 rounded-md text-white font-bold disabled:bg-indigo-400">
                    {loading ? <LoaderIcon className="mx-auto" /> : (isLogin ? 'Login' : 'Sign Up')}
                </button>
            </form>
            <p className="text-center text-gray-400">
                {isLogin ? "Don't have an account?" : "Already have an account?"}
                <button onClick={() => setIsLogin(!isLogin)} className="font-bold text-indigo-400 ml-2">
    ...
    return (
        <div className="w-full max-w-2xl p-8 space-y-6 bg-gray-800 rounded-xl">
            <div className="flex justify-between items-center">
                ...
                <button onClick={onLogout} className="flex items-center bg-red-600 hover:bg-red-700 p-2 rounded-md transition-colors duration-200">
  <PowerIcon />
</button>
...
function AdminDashboardPage({ onLogout, setPage, elections, onSelectElection }) {
    const AdminCard = ({ icon, title, description, page, action }) => (<div onClick={action || (() => setPage(page))} className="bg-gray-700 p-6 rounded-lg hover:bg-indigo-900 cursor-pointer"><div className="flex items-center space-x-4"><div className="bg-indigo-700 p-3 rounded-full shadow-lg">{icon}</div>
<div><h3 className="text-xl font-bold">{title}</h3><p className="text-gray-400 text-sm">{description}</p></div></div></div>);
  ...
  <PowerIcon />
    ...
    description="Deploy a new election to the blockchain."
    page="create-election"
/>
<AdminCard
    icon={<UsersIcon className="w-6 h-6 text-white" />}
    title="Manage Voters"
    description="Add or remove eligible voters."
    page="manage-voters"
/>
<AdminCard
    icon={<ShieldAlertIcon className="w-6 h-6 text-white" />}
    title="Fraud Detection Logs"
    description="Review suspicious vote attempts."
    page="fraud-logs"
/>
            </div>
            <div>
...
// Replace your existing handleSignUp with this exact function
const handleSignUp = async (voterId, password) => {
  setAuthLoading(true);
  try {
    // basic validation
    if (!voterId || !/^[A-Za-z0-9_-]+$/.test(voterId)) {
      alert('Enter a valid Voter ID (letters, numbers, - or _ allowed)');
      return;
    }
    ...
    // build fake email that Firebase Auth expects
    const fakeEmail = voterIdToEmail(voterId);

    // create auth user (email is the fake one)
    const userCredential = await createUserWithEmailAndPassword(auth, fakeEmail, password);

    // IMPORTANT: hardcode role to "voter" here (no undefined)
    ...
    const handleLogin = async (voterId, password) => {
        setAuthLoading(true);
        try {
            if (!voterId || !password) {
                alert('Enter your Voter ID and Password');
                return;
            }
    ...
    const connectWallet = useCallback(async () => {
        try {
            ...
            const provider = new ethers.providers.Web3Provider(instance); // <-- Use 'instance' here
    ...
    const renderContent = () => {
            ...
            switch (currentAdminPage) {
                ...
                case 'fraud-logs': return (
                    <FraudDetectionPage
                        ...
                        elections={elections.reduce((acc, election) => { acc[election.contractElectionId] = election.title; return acc; }, {})}
                    />
                );
        ...
        if (user.role === 'voter') {
            if (!selectedElection) {
                return <ElectionSelectionPage elections={elections} onSelectElection={setSelectedElectionId} onLogout={handleLogout} userEmail={extractVoterIdFromUser(user)} />;
            }
