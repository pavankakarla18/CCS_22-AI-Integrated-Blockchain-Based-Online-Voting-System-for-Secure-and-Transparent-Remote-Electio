...
import { collection, onSnapshot, addDoc, deleteDoc, doc, setDoc, getDoc, Timestamp, updateDoc } from 'firebase/firestore';
import {
...
const PowerIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18.36 6.64a9 9 0 1 1-12.73 0"></path><line x1="12" y1="2" x2="12" y2="12"></line></svg>;
...
const ShieldCheckIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><path d="m9 12 2 2 4-4"></path></svg>;
...
function VoterDashboardPage({ user, setPage, hasVoted, onLogout, election }) {
     if (!election) return (<div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-xl text-center"><LoaderIcon className="mx-auto h-10 w-10" /> <p className="mt-2 text-white">Waiting for an election to be created...</p><button onClick={onLogout} className="mt-4 bg-red-600 text-white font-bold py-2 px-4 rounded-lg">Log Out</button></div>);
    const startTime = election.startTime?.toDate().toLocaleString() || 'N/A';
    const endTime = election.endTime?.toDate().toLocaleString() || 'N/A';
    return (<div className="w-full max-w-4xl p-8 space-y-8 bg-gray-800 rounded-xl"><div className="flex justify-between items-start"><div><h2 className="text-3xl font-bold">Welcome, {user.email}</h2></div><button onClick={onLogout} className="flex items-center bg-gray-700 p-2 rounded-md"><PowerIcon /></button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="bg-gray-700 p-6 rounded-lg"><h3 className="text-xl font-semibold mb-2">{election.title}</h3><p>Start: {startTime}</p><p>End: {endTime}</p></div><div className="bg-gray-700 p-6 rounded-lg flex flex-col justify-center"><h3 className="text-xl font-semibold mb-2">Your Status</h3><p className={`font-bold ${hasVoted ? 'text-green-400' : 'text-yellow-400'}`}>{hasVoted ? 'You have voted.' : 'You have not voted.'}</p></div></div><div className="flex justify-center pt-4 gap-4"><button onClick={() => setPage('vote')} disabled={hasVoted} className="bg-indigo-600 font-bold py-3 px-8 rounded-lg disabled:bg-gray-600">{hasVoted ? 'Vote Cast' : 'Cast Vote'}</button><button onClick={() => setPage('voter-results')} disabled={!election.resultsAnnounced} className="bg-green-600 font-bold py-3 px-8 rounded-lg disabled:bg-gray-600">{election.resultsAnnounced ? 'View Results' : 'Results Not Ready'}</button></div></div>); 
}

function AdminDashboardPage({ onLogout, setPage, elections }) { 
    const AdminCard = ({ icon, title, description, page }) => (<div onClick={() => setPage(page)} className="bg-gray-700 p-6 rounded-lg hover:bg-indigo-900 cursor-pointer"><div className="flex items-center space-x-4"><div className="bg-indigo-600 p-3 rounded-full">{icon}</div><div><h3 className="text-xl font-bold">{title}</h3><p className="text-gray-400 text-sm">{description}</p></div></div></div>); 
    return (<div className="w-full max-w-4xl p-8 space-y-8 bg-gray-800 rounded-xl"><div className="flex justify-between items-start"><div><h2 className="text-3xl font-bold">Admin Dashboard</h2></div><button onClick={onLogout} className="flex items-center bg-gray-700 p-2 rounded-md"><PowerIcon /></button></div><div className="bg-gray-700 p-6 rounded-lg"><h3 className="text-lg font-semibold">Total Elections</h3><p className="text-4xl font-bold">{elections.length}</p></div><div className="space-y-4"><AdminCard icon={<PlusCircleIcon />} title="Create New Election" description="Deploy a new election to the blockchain." page="create-election" /><AdminCard icon={<UsersIcon />} title="Manage Voters" description="Add or remove eligible voters." page="manage-voters" /><AdminCard icon={<AnnounceIcon />} title="Announce Results" description="Make election results public." page="admin-announce" /></div></div>); 
}
...
function CastVotePage({ candidates, onVote, loading }) {
    const [selectedCandidate, setSelectedCandidate] = useState(null);
    return (<div className="w-full max-w-2xl animate-fade-in"><h2 className="text-3xl font-bold text-center mb-8">Select Your Candidate</h2><div className="space-y-4">{candidates.map((c, index) => (<div key={index} onClick={() => !loading && setSelectedCandidate(index)} className={`p-5 rounded-lg border-2 cursor-pointer ${selectedCandidate === index ? 'bg-indigo-900 border-indigo-500' : 'bg-gray-800 border-gray-700'}`}><div className="flex items-center justify-between"><div><h3 className="text-xl font-bold">{c.name}</h3><p className="text-indigo-300">{c.party}</p></div><div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center ${selectedCandidate === index ? 'bg-indigo-500' : 'border-gray-500'}`}>{selectedCandidate === index && <span className="text-white">✓</span>}</div></div></div>))}</div><div className="text-center mt-8"><button onClick={() => onVote(selectedCandidate)} disabled={selectedCandidate === null || loading} className="bg-green-600 disabled:bg-gray-600 font-bold py-3 px-8 rounded-lg flex justify-center min-w-[150px]">{loading ? <LoaderIcon /> : "Submit Vote"}</button></div></div>);
}

function ReceiptPage({ transactionHash, setPage }) { return ( <div className="w-full max-w-2xl p-8 space-y-6 bg-gray-800 rounded-xl text-center animate-fade-in"><ShieldCheckIcon className="mx-auto text-green-400 w-16 h-16" /><h2 className="text-3xl font-bold">Vote Confirmed</h2><p className="text-gray-300">Your vote is immutable and stored securely.</p><div className="bg-gray-900 p-4 rounded-lg"><p className="text-sm text-gray-400 mb-2">Transaction ID</p><p className="text-green-400 font-mono break-all text-sm">{transactionHash || 'Processing...'}</p></div><button onClick={() => setPage('voter-dashboard')} className="w-full bg-indigo-600 py-3 px-4 rounded-lg">Return to Dashboard</button></div>); }

function VoterResultsPage({ setPage, fetchResults, election, voteCounts, loading }) {
    useEffect(() => {
        if (election) {
    ...
    return (
        <div className="w-full max-w-2xl p-8 bg-gray-800 rounded-xl animate-fade-in">
            <button onClick={() => setPage('voter-dashboard')} className="flex items-center text-sm text-indigo-400 mb-6"><ArrowLeftIcon /> <span className="ml-2">Back</span></button>
            <h2 className="text-3xl font-bold mb-6 text-center">Official Election Results</h2>
            {loading && <LoaderIcon className="mx-auto" />}
            {!loading && election && (
                <div className="space-y-4">
                    <h3 className="text-xl font-semibold text-center">{election.title}</h3>
                    {election.candidates.map((candidate, index) => (
                        <div key={index} className="bg-gray-700 p-4 rounded-lg flex justify-between items-center">
                            ...
                            <p className="text-3xl font-bold text-indigo-400">{voteCounts[index] || 0}</p>
                        </div>
                    ))}
                </div>
            )}
...
function AdminAnnouncePage({ setPage, onAnnounce, election, loading }) {
    if (!election) return <div className="text-center"><LoaderIcon className="mx-auto" /> <p className="mt-2">Loading Election...</p></div>;
    const isElectionOver = new Date() > election.endTime?.toDate();
    return (
        <div className="w-full max-w-2xl p-8 bg-gray-800 rounded-xl text-center">
            <button onClick={() => setPage('admin-dashboard')} className="flex items-center text-sm text-indigo-400 mb-6"><ArrowLeftIcon /> <span className="ml-2">Back</span></button>
            <AnnounceIcon className="mx-auto w-16 h-16 text-indigo-400"/>
            <h2 className="text-3xl font-bold my-4">Announce Results</h2>
            <p className="text-gray-400 mb-6">This will make the results public for all voters. This action is irreversible.</p>
            ...
            <button onClick={() => onAnnounce(election)} disabled={loading || !isElectionOver || election.resultsAnnounced} className="w-full bg-green-600 text-white font-bold py-3 px-4 rounded-lg disabled:bg-gray-600">
                {loading ? <LoaderIcon className="mx-auto"/> : 'Announce Now'}
            </button>
            {!isElectionOver && <p className="text-xs text-yellow-400 mt-2">You can only announce results after the election has ended.</p>}
        </div>
    );
}
...
export default function App() {
    ...
    useEffect(() => {
        ...
        const unsubscribeElections = onSnapshot(collection(db, 'elections'), (snapshot) => {
            setElections(snapshot.docs.map(doc => ({...doc.data(), id: doc.id })));
    ...
    const handleVote = async (candidateId) => {
        ...
        if (!activeElection) return alert("No active election found.");
        
        setAuthLoading(true);
        try {
            const contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, signer);
            const tx = await contract.vote(activeElection.contractElectionId, candidateId);
            await tx.wait();
    ...
    const handleCreateElection = async (electionData) => {
        ...
        try {
            ...
            const nextContractId = await contract.nextElectionId() - 1;
            ...
            await addDoc(electionsCollection, {
                ...
                contractElectionId: parseInt(nextContractId.toString()),
    ...
    const fetchResults = async (election) => {
        if (!provider && !signer) return alert("Please connect wallet.");
        const readProvider = signer || provider;
        if (!election?.resultsAnnounced) return alert("Results have not been announced.");
        setAuthLoading(true);
        try {
            ...
            const formattedCounts = counts.map(count => parseInt(count.toString()));
            ...
            {!user ? (
                ...
                (() => {
                    ...
                    switch (currentAdminPage) {
                        ...
                        case 'admin-announce': return <AdminAnnouncePage setPage={setPage} onAnnounce={handleAnnounceResults} election={activeElection} loading={authLoading} />;
                        default: return <AdminDashboardPage onLogout={handleLogout} setPage={setPage} elections={elections} />;
                    }
                })()
            ) : ( // Voter Role
                (() => {
                    ...
                    switch (currentVoterPage) {
                        case 'vote': return <CastVotePage candidates={activeElection?.candidates || []} onVote={handleVote} loading={authLoading} />;
                        case 'receipt': return <ReceiptPage transactionHash={transactionHash} setPage={setPage} />;
                        case 'voter-results': return <VoterResultsPage setPage={setPage} fetchResults={fetchResults} election={activeElection} voteCounts={voteCounts} loading={authLoading} />;
                        default: return <VoterDashboardPage user={user} setPage={setPage} hasVoted={hasVoted} onLogout={handleLogout} election={activeElection} />;
                    }
                })()
            )}
        </div>
