    ...
    return (
        <div className="w-full max-w-2xl p-8 space-y-6 bg-gray-800 rounded-xl">
            <div className="flex justify-between items-center">
                <div>
                    {/* --- NEW FEATURE: Clearer title for voter dashboard --- */}
            ...
            <div className="border-t border-gray-700 pt-4">
                <h3 className="text-xl font-semibold mb-2 text-white">Available Elections</h3>
                <p className="text-sm text-gray-400 mb-4">Select an election to view details or cast your vote.</p>
                <div className="space-y-4 max-h-96 overflow-y-auto">
                    {elections.length > 0 ? elections.map(election => (
                        <div key={election.id} onClick={() => onSelectElection(election.id)} className="bg-gray-700 p-4 rounded-lg hover:bg-indigo-900 cursor-pointer">
                            <h3 className="text-xl font-semibold">{election.title}</h3>
                            <p className="text-sm text-gray-400">Ends: {election.endTime?.toDate().toLocaleString()}</p>
                        </div>
                    )) : (
                        <p className="text-center text-gray-400">No elections are available at the moment.</p>
                    )}
                </div>
            </div>
        </div>
...
function AdminDashboardPage({ onLogout, setPage, elections, onSelectElection }) {
    ...
    return (<div className="w-full max-w-4xl p-8 space-y-8 bg-gray-800 rounded-xl"><div className="flex justify-between items-start"><div><h2 className="text-3xl font-bold">Admin Dashboard</h2></div><button onClick={onLogout} className="flex items-center bg-gray-700 p-2 rounded-md"><PowerIcon /></button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6"><AdminCard icon={<PlusCircleIcon />} title="Create New Election" description="Deploy a new election to the blockchain." page="create-election" /><AdminCard icon={<UsersIcon />} title="Manage Voters" description="Add or remove eligible voters." page="manage-voters" /></div><div><h3 className="text-xl font-semibold mb-4">Manage Existing Elections ({elections.length})</h3><div className="max-h-96 overflow-y-auto bg-gray-900 p-2 rounded-md space-y-2">{elections.map(e => (<div key={e.id} onClick={() => onSelectElection(e.id)} className="bg-gray-700 p-4 rounded-lg hover:bg-indigo-900 cursor-pointer flex justify-between items-center"><div><p className="font-bold">{e.title}</p><p className={`text-xs font-bold ${e.resultsAnnounced ? 'text-green-400' : 'text-yellow-400'}`}>{e.resultsAnnounced ? 'Results Announced' : 'Results Pending'}</p></div><button className="text-sm bg-indigo-600 px-3 py-1 rounded">Manage</button></div>))}</div></div></div>);
}
...
function CastVotePage({ candidates, onVote, loading, election, onBack }) {
    ...
    return (<div className="w-full max-w-2xl animate-fade-in"><button onClick={onBack} className="flex items-center text-sm text-indigo-400 mb-6"><ArrowLeftIcon /> <span className="ml-2">Back to Details</span></button><h2 className="text-3xl font-bold text-center mb-8">Select Your Candidate</h2>{!isElectionActive && <p className="text-center text-red-400 mb-4">Voting is currently closed for this election.</p>}<div className="space-y-4">{candidates.map((c, index) => (<div key={index} onClick={() => !loading && isElectionActive && setSelectedCandidate(index)} className={`p-5 rounded-lg border-2 cursor-pointer ${selectedCandidate === index ? 'bg-indigo-900 border-indigo-500' : 'bg-gray-800 border-gray-700'} ${!isElectionActive ? 'opacity-50 cursor-not-allowed' : ''}`}><div className="flex items-center justify-between"><div><h3 className="text-xl font-bold">{c.name}</h3><p className="text-indigo-300">{c.party}</p></div><div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center ${selectedCandidate === index ? 'bg-indigo-500' : 'border-gray-500'}`}>{selectedCandidate === index && <span className="text-white">✓</span>}</div></div></div>))}</div><div className="text-center mt-8"><button onClick={() => onVote(selectedCandidate)} disabled={selectedCandidate === null || loading || !isElectionActive} className="bg-green-600 disabled:bg-gray-600 font-bold py-3 px-8 rounded-lg flex justify-center min-w-[150px]">{loading ? <LoaderIcon /> : "Submit Vote"}</button></div></div>);
}
...
export default function App() {
    ...
    const handleVote = async (candidateId) => {
        ...
        if (!selectedElection) return alert("No election selected.");
    ...
    const handleRemoveUser = async (userId) => {
        alert("This will only remove the user from the Firestore database, not from Firebase Authentication. A secure implementation requires a backend Cloud Function to properly delete users.");
